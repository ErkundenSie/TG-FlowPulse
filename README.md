...
- 私聊自动回复支持两种触发方式：填写关键词/正则后命中才回复，或开启 `任意私聊消息都触发` 后在指定时间段内收到私聊就回复
- 自动回复可选择固定文本模板（支持 `{keyword}`、`{message}`、`{sender}` 等变量）或开启 `AI 自动回复`；AI 使用系统设置里的 OpenAI 兼容接口配置，可额外填写回复提示词
